...
const mockXpub =
  'tpubDCFR6dz4WRkaM4qwqVSpqfREKZYo4VMLRmahh2Jf3KWL4YsU7NbvjhoXK6K6A3XhpPWTLWbinLCK5EhwB3KodoUafpwjsqbCo51NyYj2427'
const mockXpriv =
  'tprv8ZgxMBicQKsPfQaqmQ4u2nQxV124k4uta7sECKQiSoYLtt6UC7dmRN4ZYC2jqRhRhWXwqpXzjeqGDvt6RHBxqD45i77edr1zqLrFB5CH1wC'
const mockTxHash = '00000000000000000011d0cd37d89efcd1efdce2ee97ef3565094f9c71d2cb0c'
const mockRawTx =
  ''
...
describe('constructor', () => {
  ...
  it('should throw given invalid wallet type', () => {
    const walletData = JSON.parse(
      JSON.stringify({ ...basicBitcoinWalletData, walletType: 'invalid' })
    )
    expect(() => {
      // eslint-disable-next-line no-new
      new WalletBitcoin(walletData)
    }).toThrowError(new Error('Invalid walletType: invalid'))
  })
...
describe('generateWallet', () => {
  ...
  it('should generate ledger wallet', async () => {
    ...
    expect(walletBitcoin.getWalletData()).toEqual({
      ...basicBitcoinWalletData,
      walletType: 'ledger'
    })
  })
...
describe('encryptAccount', () => {
  ...
  it('should throw when xpriv does not exist', async () => {
    ...
    await expect(walletBitcoin.encryptAccount('password')).rejects.toThrow(
...
describe('decryptAccount', () => {
  ...
  it('should throw if password not match', async () => {
    ...
    walletBitcoin.clearPrivateKey()
    walletBitcoin.walletData.accounts[0].hdWalletVariables.xpub = null
    await expect(walletBitcoin.decryptAccount('wrongPassword')).rejects.toThrow(
  ...
  it('should throw if EncryptedPrivateKey not exist', async () => {
    await expect(walletBitcoin.decryptAccount('wrongPassword')).rejects.toThrow(
      new Error('EncryptedPrivateKey does not exist')
    )
  })
})
...
describe('retrieveAddress', () => {
  ...
  it('should throw if trying to retrieve address other than from ledger', async () => {
    await walletBitcoin.generateWallet({ walletType: 'drive', cryptoType: 'bitcoin' })
    await expect(walletBitcoin.retrieveAddress()).rejects.toThrow(
      new Error('Cannot retrieve address for drive')
    )
  })
})
...
describe('sync', () => {
  ...
  it('should sync drive wallet', async () => {
    ...
    expect(hdWalletVariables.addresses[0]).toEqual({
      address: mockAddress,
