...
type WalletDataEthereum = {
  walletType: string,
  accounts: AccountEthereum[]
}

type WalletDataBitcoin = {
  walletType: string,
...
interface IWallet<WalletData, Account> {
...
class WalletBitcoin implements IWallet<WalletDataBitcoin, AccountBitcoin> {
  ledger: any
  ...
  getTxFee = async ({ to, value }) => {
    ...
    let costInStandardUnit = utils
      .toHumanReadableUnit(costInBasicUnit, 8, 3)
      .toString()
    return { price, gas, costInBasicUnit, costInStandardUnit }
  }

  sendTransaction = async ({ to, value, txFee }) => {
    let account = this.getAccount()
    const satoshiValue = parseFloat(value) * 100000000 // 1 btc = 100,000,000 satoshi
